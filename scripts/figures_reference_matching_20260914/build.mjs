...
const NOTES = {
  ...
  fig4_effects_interaction:
    "Figure 4. (a) representation-swap effects E = P(new) - P(control) for the pre-specified S and D branches, each with the unadjusted 95% paired bootstrap interval of its source file. (b) the direct interaction I = E_L - E_J for every encoder instance of the extra slot; the MPDD and BTAD rows carry the per-encoder S, D, E1, E2 and E3 lanes, and the MVTec and VisA rows are drawn from the four-dataset aggregation generalization_mvtec_visa_20260915/interaction_generalization.csv as the GEN lane (bootstrap mean with its 95% bootstrap interval). Sources: 05_extra_encoders/encoder_comparison_three.csv (S, D, E1, E2, E3), 02_interaction/representation_effects.csv (S), 04_new_encoder/representation_effects_new_encoder.csv (D) and generalization_mvtec_visa_20260915/interaction_generalization.csv (MVTec/VisA, four-dataset aggregation). Bars and points are point estimates; S and D intervals are the unadjusted 95% paired bootstrap intervals of the source files, the E1/E2/E3 intervals are that file's 95% level (its 98.75% level is the family-adjusted one). S uses 12 MPDD conditions and 8 BTAD conditions, D and the E branches use 4 conditions (seed {0,1} x K {1,4}), so the rows are not on identical grids and this is stated rather than hidden. BTAD uses the corrected geometry revision. The dashed guide marks 0.005 macro pixel AP, a project-internal practical scale, not an industrial standard.",
