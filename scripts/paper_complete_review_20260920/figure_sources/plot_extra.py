...
for i,(ds,con,col) in enumerate([('mpdd','I_TRI','#2e6f9e'),('mpdd','I_BAL','#91b9d0'),('btad','I_TRI','#b27c20'),('btad','I_BAL','#e5c590')]):
    construction='TRI' if con=='I_TRI' else 'BAL';axs[0].bar(x+(i-1.5)*.19,[a['interaction_percentage_points'][f'{ds}|{con}|{v}'] for v in abl],.18,color=col,label=ds.upper()+' '+CONSTRUCTION_LABELS[construction])
...
for ax,title,yl in zip(axs,['(a) Matching interactions','(b) Mean pixel AP under joint matching'],['Interaction (AP points)','Pixel AP']):
    ax.set_xticks(x,[ABLATION_LABELS[v] for v in abl]);ax.set_title(title,loc='left',fontweight='bold');ax.set_ylabel(yl);ax.grid(axis='y',alpha=.2);ax.set_axisbelow(True)
...
src=src.replace('B/S canvas (cropped)', 'DINOv2-B/14 + DINOv2-S/14 canvas')
...
s.CASE_ROWS_PER_PAGE=2;s.CASE_TOP_IN=.32
src=inspect.getsource(s._build_case_page)
src=re.sub(
    r'    suptitle = .*?    top = CASE_TOP_IN',
    "    suptitle = \"Interaction cases (seed 0, K = 4): Baseline = Dual-encoder baseline; Joint and Independent matching shown\"\n"
    "    top = CASE_TOP_IN",
    src,
    flags=re.S,
)
...
def case_row(*args):
    ...
    row['panels'][1]=('mask',(row['panels'][1][2]>0).astype(float),None)
    row['heading']=row['heading'].replace('I_TRI', INTERACTION_LABELS['I_TRI']).replace('I_BAL', INTERACTION_LABELS['I_BAL']).replace('most_positive','positive extreme').replace('most_negative','negative extreme')
    return row
