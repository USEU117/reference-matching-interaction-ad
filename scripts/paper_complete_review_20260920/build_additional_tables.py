...
for dataset in names:
    for method in ('A1_J','A1_L'):
        ...
        image_rows.append([names[dataset],method.replace('_',' ')]+[f'{v:.4f}' for v in means])
...
labels={'controlled_A1_J':'A1 J','controlled_A1_L':'A1 L','anomalydino_canvas':'AnomalyDINO','anomalydino_canvas_rotation':'AnomalyDINO + rotation','PatchCore_harmonised448':'PatchCore 448'}
...
tables['harmonised']={
 ...
 'rows':harmrows,'widths':[2.5,5,2.5,2.5,4.5],'left_cols':[0,1], 'bold_rows':[i for i,r in enumerate(harmrows) if r[1].startswith('A1')],
