...
def build_doc() -> Path:
    ...
    for line in ["作者：________________", "学校 / 学院：________________", "专业：________________", "评审人：________________"]:
        rp = p3.add_run(line + "\n")
        rp.font.size = Pt(12)
    ...
    add_table(
        ...
        [
            ["P1 初稿修订", "按外部评审意见修改结构与表述", "CPU", "章节逻辑、图表编号、引用一致", "形成 V0.2"],
    ...
    add_body(doc, "本文件是基于当前真实实验状态形成的 V0.1 初稿，适合用于确定论文主线、与外部评审讨论章节结构，并作为后续补实验的占位框架。作者、学校、专业和评审人信息需要在封面补充。参考文献已按当前可核对信息整理，提交学校前仍需按照学校指定的 GB/T 7714 或模板格式统一。")
