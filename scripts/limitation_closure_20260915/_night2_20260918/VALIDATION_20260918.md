...
本报告验收之后（2026-09-19 下午）另做了一轮补齐：VisA 的 AnomalyDINO 逐样本 dump 补到 48/48、S8 四数据集共同区域表重算为 864 行且四数据集各 6 个方法列、图 7 的 VisA 12 类按 6 列重出、工作流 B 的 BTAD 补跑四变体 + ε 网格、`A_btad03_corrected` 的四门校验值补全、资源对比表刷新、KSDD2 文献补全卷号/文章号与 DOI——各项证据见 `AUDIT_CLOSURE_20260919.md` §五与 §七。这些都不改变本报告正文的任何实测数字与判定，**本报告的 36 项检查结论与 verdict=pass 未变**。
