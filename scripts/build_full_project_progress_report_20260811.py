...
def build():
    ...
    timeline = [
        ...
        ("8 月 10 日", "论文与文献", "完成中文论文 V0.1、英文 学术风格 V0.2/V0.3，改进主图，扩展到 37 篇参考文献并整理 2026 年相关工作。"),
    ...
    paper_rows = [
        ...
        ("英文 英文初稿 V0.2", "完成", "期刊中性单栏格式；摘要、关键词、声明、数据/代码可用性等齐全。"),
        ("英文 英文初稿 V0.3", "完成", "重新制作主图，扩充并融入代表性与 2026 年文献，当前共 37 篇参考文献。"),
    ...
    add_body(doc, "上面的百分比只是项目管理估算，不是数学公式。它反映“距离一篇较完整 投稿稿还差多少”，不是代码文件数量。")
    ...
    add_body(doc, "适合希望尽快完成一篇客观、规范的 英文稿。重点不再追求“动态融合一定提高”，而是把论文主线收紧为：少样本异构分数校准为什么会失败，如何建立无泄漏、安全回退和独立验证流程。")
    for text in [
