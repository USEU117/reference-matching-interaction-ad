...
Here K ∈ {1, 2, 4}. Support selection requires verified normality and product-category identity. Query ground-truth labels and pixel masks are used for evaluation, not for feature extraction, memory construction, query scoring, or parameter selection on the external-validation datasets. Development-set labels inform method selection as described in Section 4.1.1. All target-category model parameters remain fixed. Building a memory from normal images is a data-dependent preparation step, even though it performs no gradient optimization.
