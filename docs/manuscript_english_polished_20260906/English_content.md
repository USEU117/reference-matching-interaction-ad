...
Industrial inspection often begins with few defect-free images and limited knowledge of possible defects. A single pretrained representation may miss local cues, while heterogeneous features differ in spatial grids and magnitudes. We propose DCFnet, a Dual-Encoder Calibrated Fusion Network for normal-only few-shot industrial anomaly localization. It combines frozen patch descriptors from DINOv2, a self-supervised visual foundation model, and the image encoder of Contrastive Language–Image Pre-training (CLIP), a model pretrained on image–text pairs. Grid alignment and branch-wise normalization enable fixed equal-weight concatenation and joint normalization. Normal support descriptors form a memory for nearest-neighbor anomaly scoring, without target-domain parameter optimization or text-based inference. Experiments on four industrial benchmarks show improved mean pixel-level localization over a matched DINOv2-only baseline, with category-specific losses and additional computational cost. Code is available at https://github.com/USEU117/reference-matching-interaction-ad.
...
Reconstruction and one-class methods learn inspection-specific representations or decision rules from normal data. Their advantage is the ability to shape features toward the detection task. The following examples achieve this through different forms of supervision and augmentation, which must be considered when comparing them with a detector whose target-category encoders remain frozen.

SLSG combines generative pretraining, simulated anomalies, and self-attentive graph convolution to learn normal embeddings and their spatial relationships [1]. RealNet uses diffusion-based anomaly synthesis, anomaly-aware feature selection, and reconstruction-residual selection to reduce feature redundancy and expose defect-sensitive residuals [6]. These mechanisms provide task-specific supervision, but their effectiveness depends in part on how well synthesized abnormalities represent useful departures from normality. Dual-path Frequency Discriminators exploit spatial and frequency views with pseudo-anomaly supervision to make subtle defects more distinguishable [7]. DMMGNet separates augmented training features from stored memory features and uses discrimination mapping and memory-bank mean guidance to support few-shot learning [2]. The latter two methods illustrate how representation learning can address sparse normal data, while retaining a model-fitting stage. DCFnet avoids this target-category optimization by fixing its encoders and feature transformation; the corresponding limitation is that it cannot learn a category-specific correction when the transferred descriptors are inadequate.

Pretrained feature embedding and normal-memory methods measure deviations directly in a transferred feature space. PaDiM represents normal patch embeddings with multivariate Gaussian distributions [8]. This provides a compact statistical description, but estimating a high-dimensional covariance from very few examples is challenging. PatchCore stores a coreset of normal local descriptors and applies nearest-neighbor scoring [9]. Retaining observed descriptors avoids prescribing a single Gaussian distribution, although retrieval cost and reference coverage become important. Both approaches avoid learning named defect classes. DCFnet follows the descriptor-memory approach and retains all available support patches, trading greater reference retention for memory and search cost. With few references, neither a statistical model nor a descriptor memory can be assumed to cover every acceptable appearance variation.

Few-shot work has consequently focused on expanding normal coverage or improving representation quality. Hu et al. transform support features conditionally and construct multi-frequency descriptors to compensate for sparse normal patterns in FEAD [10]. Wei et al. formulate K-NG for few-shot online detection and update a Neural Gas representation from an unlabeled stream while attempting to avoid contamination by anomalies [11]. Zhou et al. combine global invariant and multiscale local features and explicitly calibrate differently distributed anomaly scores in PGAD [4]. Li et al. refine normal prototypes at test time in FastRef and use optimal transport to reduce the risk of absorbing anomalous query features [12]. Feature transformation and multiscale modeling enrich the evidence available from sparse supports, whereas online updating and test-time refinement can accommodate appearance changes. Updating from unlabeled queries also requires safeguards against incorporating anomalies into the normal representation. DCFnet instead keeps the normal memory fixed and asks whether a second frozen representation provides a reproducible gain without query-conditioned adaptation.

Foundation-model and hybrid approaches build on transferable pretrained representations. Self-supervised visual foundation models have substantially changed the quality of transferable local descriptors. AnomalyDINO applies frozen DINOv2 patches to few-shot anomaly detection and shows that straightforward patch comparison is already highly competitive [3]. UniVAD develops a training-free unified model that reasons over reference images across industrial, logical, and medical anomalies [13]. SubspaceAD fits a principal subspace to frozen DINOv2 patches and scores reconstruction residuals, providing a strong training-free alternative to direct nearest-neighbor memories [14]. DCP-SFR argues that deep structural representations may suppress shallow defect cues and refines them to preserve subtle anomalies [15]. Collectively, these studies show that feature selection, normal-space modeling, and cue preservation can be as important as the downstream anomaly scorer. They also establish a demanding visual-only baseline against which fusion must be assessed.

Vision-language pretraining offers another route to transfer. WinCLIP uses prompt ensembles, window-level visual features, and optional normal references for zero- and few-shot anomaly classification and segmentation [16]. AnomalyCLIP replaces object-specific descriptions with object-agnostic prompts learned for normality and abnormality [17], whereas PromptAD learns prompts using only normal target samples [18]. InCTRL performs in-context residual learning with few-shot sample prompts [19]. AA-CLIP introduces anomaly-aware text anchors and patch-level visual alignment [20], and FAPrompt decomposes coarse abnormality descriptions into fine-grained prompt components [21]. Language prompts provide transferable descriptions of normality and abnormality, which is useful when visual references are scarce. Their scores can also depend on prompt design or learned prompt parameters; target-normal prompt tuning introduces an additional adaptation stage. These dependencies differ across the cited methods and should not be equated with target-defect supervision. DCFnet uses only the frozen CLIP image encoder with the diagonally prominent attention map (DPAM) modification from AnomalyCLIP [17] at inference; it is therefore a dual-visual-encoder method rather than a vision-text inference system.

Hybrid approaches are the closest context for DCFnet. Ma et al. combine retrieval-enhanced references and multimodal prompt fusion in ReMP-AD [22]. Xu et al. study cooperative refinement of multimodal features for few-shot detection [23]. Guo et al. use DINOv2-guided patch matching and a learned decoder in Sea-CLIP [5]. PAPL introduces particle-based adaptive prompt learning for zero-shot inspection [24], while Jiang et al. combine CLIP and DINOv2 with multimodal fusion and stabilized attention pooling [25]. These designs can couple complementary representations through retrieval, prompting, or learned fusion, but the resulting system gains do not isolate the effect of fixed visual-feature concatenation. Pairing CLIP and DINOv2 is therefore not itself a novel contribution. DCFnet makes the distance contribution of each frozen branch explicit and preserves a fixed reference memory, at the expense of a second backbone and without the flexibility of a learned fusion rule.
...
Here x_i^c is the ith normal support image of category c, i = 1, …, K, and K ∈ {1, 2, 4}. Support selection requires verified normality and product-category identity. Query ground-truth labels and pixel masks are used for evaluation, not for feature extraction, memory construction, query scoring, or parameter selection on the external-validation datasets. Development-set labels inform method selection as described in Section 4.1.1. All target-category model parameters remain fixed. Building a memory from normal images is a data-dependent preparation step, even though it performs no gradient optimization.
...
Here G_D(x_i^c) denotes the DINO patch lattice of the category-c support image x_i^c, and q indexes a patch on that lattice. Descriptors are stored in a FAISS IndexFlatL2 index [28]. For query patch p, the raw anomaly score is one half of the minimum squared Euclidean distance:
...
| CLIP visual | ViT-L/14; 24 blocks; width 1024; 16 heads; input 518 × 518 | 37 × 37 × 768 after projection; 0 |
...
The validated ablations concern encoder presence, reference count, and reference sampling. Matched component studies of alternative spatial alignment, branch normalization, DPAM, and feature fusion versus separately searched score fusion remain incomplete. Accordingly, the study establishes the effect of the complete fusion path, not a separately measured gain for every calibration operation. Under the positive-norm, equal-weight assumptions of Section 3.4, the concatenated branch-unit descriptors have constant norm. Joint normalization therefore fixes the distance scale; it is not expected to improve ranking-based metrics independently in exact arithmetic.
...
The implementation and reproducibility materials are available at https://github.com/USEU117/reference-matching-interaction-ad. Within the repository, docs/submission_reproducibility_20260826/README.md is the reproduction entry point, and submission_repro_20260827 contains the archived evidence package. Its SOURCE_COMMIT and checksum manifests identify the experiment archive; they do not identify later manuscript edits. The evaluated data are MPDD [29], BTAD [30], MVTec AD [31], and VisA [32], distributed by their respective providers. The repository records the required dataset layout and checkpoint identifiers without redistributing the original images or third-party model weights.
