...
The direction is robust; the zero-exclusion decision is not. Across the seven settings, the canvas rule, Procrustes, the permutation and four regularizer values including the exact assignment, all fourteen point estimates are positive. The exact assignment is in fact stronger than the canvas rule, and only the soft mixing at the intermediate regularizer values has intervals that span zero.

Two observations localize the cause, and we state them as observations rather than as a mechanism. First, the exact assignment uses the same cost matrix and does not flip the decision, so replacing one data-driven correspondence with another does not by itself remove the interval separation. Second, the failure appears only in a middle band of the regularizer and disappears again when the mixing is made wider, a non-monotone pattern for which we offer no mechanism-level explanation. What distinguishes the soft mixing from the other three conventions is that it also changes the multiset of the C descriptors, since each row becomes a weighted combination of roughly 15 to 40 canvas positions, with a median row-maximum weight of 0.044. The most defensible statement is therefore that the sign of the interaction does not depend on the canvas-correspondence choice, while its interval separation is not robust to a transformation that additionally smooths the C branch.

The transport plans themselves carry almost no canvas-position information: the median diagonal mass is 0.000949 against a chance value of 0.000977, and the fitted Procrustes map sits at a Frobenius distance of about 39 from the identity, so neither solves a position-level correspondence. We also record the verification of the new variant: it recovers a known synthetic assignment exactly, it reproduces the stored patch scores when the plan is forced to the identity with a maximum absolute difference of 6.4e-07, and the three earlier variants reproduce to at most 1.8e-06 once the new code path is present. The harness also reproduces the S branch's sign and zero-exclusion decisions on both datasets, with values differing from the study's by up to 0.07 point, so it is adequate for comparing correspondence conventions; its BTAD numbers nevertheless belong to this scoring chain and must not be mixed with the D, E1 or E2 numbers.

On BTAD only the canvas rule and the soft mixing were run. Both fail to exclude zero for either contrast, with the canvas rule at +0.001 points [−0.302, +0.259] for $I_{TRI}$ and −0.080 [−0.317, +0.164] for $I_{BAL}$, and soft mixing at +0.115 [−0.221, +0.505] and +0.103 [−0.233, +0.630]. On this dataset the correspondence swap therefore does not change the judgement, which was already "no evidence of a direction".
