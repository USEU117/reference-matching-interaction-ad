...
> **后续指针（2026-09-23，只加指针、不改原文）**：本报告 §A.4 对"三处同批图件目录"给出的是**建议**（当时未执行）。该建议已由后续任务落成实际归档，见 [`FOLDER_CONSOLIDATION_20260923.md`](FOLDER_CONSOLIDATION_20260923.md)：`docs/` 的 5 个历史图件目录与 6 个历史稿件/评审目录已用 `git mv` 归并到 `docs/archive_pre202609/`（含 `figures/` 子目录），共 52 个文件、0 删除、可逆；现役图件目录 `docs/figures_reference_matching_20260914/` 未动。
