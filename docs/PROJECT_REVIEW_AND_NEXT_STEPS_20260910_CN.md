...
历史 81、122、123、141 个测试通过记录来自不同时间或范围，其中也存在文档不一致。统一一份带日期、解释器及明确 `tests` 范围的记录即可，不要把这些数字混成当前 HEAD 的验收结果。另应把较新的 MPDD / BTAD 许可核查同步到正式发布说明；冻结旧包保留历史身份，正式 release 使用新的文件清单与哈希。详细入口和差异见 [复现审计附录](D:/STUDY/My_github/sci_project/docs/project_review_20260910/repro_audit.md)。
