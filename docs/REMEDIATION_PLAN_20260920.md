...
> **2026-09-23 校·已收口**：本文件全部条目（P0-1…P0-4、P1-1…P1-7、P2-1…P2-6、P3-1…P3-6、§"需要作者提供的信息" 1–12、§"只作为论文限制保留" R-18…R-21、检查清单）已并入 `docs/MASTER_TODO_PAPER_PPT_FIGURES_20260923.md`，**逐条映射见该表 §十一 11.2**（含"已完成 / 仍待办 / 待拍板 / 作者信息"判定与新旧编号对应）；**本文件转为历史，不再作为待办入口**（原文一律不改写）。
>
...
- **收尾轮（2026-09-23，原文不改写）**：`selfcheck.py`（对应 **E-10 / F22**）已按"先备份 + 记 SHA-256 → 实跑 → 立即逐字节还原"执行，实测 **67/69**（2 项为已登记既有失败：`_smoke` 两个 `.npz` 缺失 + `REPORT_CN.md` 仅 mtime 变化；旧提纲 docx 不在盘），还原后两份 JSON 哈希与运行前**完全一致**、`git status --porcelain -- experiments data` 为空；`build.py` 版式母本 `9DB99E60…` 与两张冻结表 `3C83AB00…` / `1C770129…` 全程不变。另：清单 **A-07 / A-17** 像素级复核**通过**（图 2(b) 紫框与填色格同列 100% 重合；deck 第 23/24 页位图与图 S4 两页 PNG **逐字节 + 逐像素相同**）、**图 1 生成器已纳入版控**（`scripts/main_figure_20260920/`，复现 `powershell -File scripts/main_figure_20260920/run_pipeline.ps1`，与入稿图逐字节相同）；**R-01…R-21 的已处置/待作者状态不变**，仅新增 1 条**清单外新发现 N-1**（`plot_primary.py` 数据来源换表 ⇒ 入稿图 fig4b 与 Table 15 四条件点值不一致，待作者二选一）。详见 `docs/FINAL_REPAIR_AND_ACCEPTANCE_20260923.md` §8、`docs/REVIEW_CHECKLIST_FOR_REGENERATED_PAPER_20260923.md` §5.4/§7。
...
- **2026-09-23｜最终轮指针（本轮由收口任务写入，详见 `docs/论文与图件问题汇总_仅复核_20260921.md` §八·续）**：
  - **A 线已完成**：Data and Code Availability 与摘要的 URL 矛盾已消除（T15）；命名温和版落地（Table 1/2 `Full name` 列 + 首现加粗 + 图注全称，T16）；Table 12 表注已按最保守表述写 `auxiliary-domain-trained prompt learner`（T17）。
  - **B 线已完成**：同口径 448 子集实验与协议杠杆实算、图 S6 全部落盘并过门禁（T18/F18），`05_baselines_harmonised_20260922/**`。
