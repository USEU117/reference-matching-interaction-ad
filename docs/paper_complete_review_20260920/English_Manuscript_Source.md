...
Figure 1. Controlled framework and representation comparisons. (a) K normal support images $x_i^c$ form a fixed, aligned reference bank. (b) The query follows the same frozen feature path. Joint matching selects one shared reference row; independent matching selects a row independently in each branch. For either matching rule, shared resizing and Gaussian smoothing produce the continuous map $A_t$, whose maximum is the image score. The second image panel is a thresholded display of the same map, not another learned output. (c) The duplicate-branch control isolates reweighting; the equal-weight replacement and the balanced replacement define the representation effects and their matching interactions. DINOv2-B/14 and AnomalyCLIP visual provide the baseline features; DINOv2-S/14 and WideResNet50-2 are the replacement features. The displayed map is the archived Dual-encoder baseline result under joint matching for MPDD metal_plate/test/scratches/026.png at seed 0 and K = 1. Multiple support thumbnails illustrate the general input, not that example's support count. The contour uses 256-bin Otsu thresholding solely for visualization.
...
Figure 2. Joint and independent matching on the same fixed bank. (a) One query patch and the candidate reference rows r = 1, ..., 8; the shaded cell in each branch row marks the candidate that branch finds closest. The cells are ordering indicators only and carry no measured value. (b) Joint matching requires a single shared reference row for all branches and then minimizes the weighted sum of distances over candidates, whereas independent matching lets each branch take its own nearest row before the weighted sum is formed. Both panels use the same aligned candidate set, the same branch descriptors and the same nonnegative weights that sum to one, so the two panels differ only in the order of combination and candidate minimization. (c) The ordering constraint $L(p)$ less than or equal to $J(p)$ follows from minimizing over a less restricted reference assignment. The gap is a raw-score difference, not an AP improvement.
...
| Construction | Description | Distance weights | Controlled comparison |
| --- | --- | --- | --- |
| Dual-encoder baseline | Dual-encoder baseline (DINOv2-B/14 + AnomalyCLIP visual) | DINOv2-B/14 1/2; AnomalyCLIP visual 1/2 | Fixed dual-encoder baseline |
| Duplicate-branch control | Exact duplicate of the DINOv2-B/14 descriptor; no new encoder | DINOv2-B/14 1/3; DINOv2-B/14 copy 1/3; AnomalyCLIP visual 1/3 | Reweight existing information; no new encoder |
| Equal-weight replacement | Equal-weight replacement (DINOv2-S/14 or WideResNet50-2 in the copied slot) | DINOv2-B/14 1/3; DINOv2-S/14 or WideResNet50-2 1/3; AnomalyCLIP visual 1/3 | Replace the duplicate at fixed slot weights |
| Balanced replacement | Balanced replacement (DINOv2-S/14 or WideResNet50-2 in a quarter slot) | DINOv2-B/14 1/4; DINOv2-S/14 or WideResNet50-2 1/4; AnomalyCLIP visual 1/2 | Preserve AnomalyCLIP visual and total non-AnomalyCLIP visual weights |

Each construction is evaluated under joint and independent matching. Bold identifies the dual-encoder baseline, not a claim of best performance. All encoder parameters are frozen. For DINOv2-S/14, balanced replacement also preserves the combined DINO-family weight.
...
Figure 3. Fixed representation constructions and the contrasts built from them. (a) The Dual-encoder baseline, Duplicate-branch control, Equal-weight replacement and Balanced replacement with their distance weights; a slot keeps the same colour across panels. (b) Moving from the Dual-encoder baseline to Duplicate-branch control changes only how the fixed weights are split, raising the effective DINOv2-B/14 weight from 1/2 to 2/3 and lowering the AnomalyCLIP visual weight from 1/2 to 1/3, with no new representation added, and is therefore the weight-confounding control; moving from Duplicate-branch control to Equal-weight replacement puts a real encoder into the slot at an unchanged weight; moving from the Dual-encoder baseline to Balanced replacement keeps the combined weight of the non-AnomalyCLIP visual branches and the AnomalyCLIP visual weight equal. (c) The four contrasts estimated in this paper. The Equal-weight replacement-to-Dual-encoder baseline difference moves both factors at once and is never attributed to DINOv2-S/14 alone. The extra slot is instantiated by five frozen encoders: DINOv2-S/14 and WideResNet50-2 were pre-specified, while DINO ViT-S/8, ConvNeXt-Tiny and Swin-Tiny were added after those results were known and are exploratory transfer checks. These are analysis configurations in one frozen pipeline, not trained networks.
...
$$
E_{\mathrm{Equal},t}=P(\mathrm{Equal}_t)-P(\mathrm{Duplicate}_t)\tag{6}
$$

$$
E_{\mathrm{Balanced},t}=P(\mathrm{Balanced}_t)-P(\mathrm{Baseline}_t)\tag{7}
$$
...
$$
I_{\mathrm{Equal}}=E_{\mathrm{Equal},\mathrm{L}}-E_{\mathrm{Equal},\mathrm{J}}\tag{8}
$$

$$
I_{\mathrm{Balanced}}=E_{\mathrm{Balanced},\mathrm{L}}-E_{\mathrm{Balanced},\mathrm{J}}\tag{9}
$$
...
$$
\Delta I_q=I_{q,\mathrm{WideResNet50-2}}-I_{q,\mathrm{DINOv2-S/14}},\quad q\in\{\mathrm{Equal},\mathrm{Balanced}\}\tag{10}
$$
...
| Encoder or component | Input and retained feature path | Descriptor / target training |
| --- | --- | --- |
| DINOv2-B/14 visual encoder (frozen) | DINOv2 ViT-B/14; final patch features; short side 448 | 768 dimensions; frozen |
| DINOv2-S/14 visual encoder (frozen) | DINOv2 ViT-S/14; final patch features; short side 448 | 384 dimensions; frozen |
| AnomalyCLIP visual branch (frozen) | CLIP ViT-L/14 visual + DPAM; 518 × 518; retain layer 24 after projection | 768 dimensions; frozen |
| WideResNet50-2 branch (frozen) | WideResNet50-2; ImageNet1K V1; layer2 + layer3 on DINOv2-B/14 canvas | 512 + 1024 = 1536; frozen |
| DINO ViT-S/8 encoder (frozen, exploratory) | Original DINO ViT-S/8; final patch features | 384 dimensions; frozen |
| ConvNeXt-Tiny encoder (frozen, exploratory) | ConvNeXt-Tiny; two retained stages | 576 dimensions; frozen |
| Swin-Tiny encoder (frozen, exploratory) | Swin-Tiny; two retained stages | 576 dimensions; frozen |
| Shared spatial alignment | Shared canvas alignment; common DINOv2-B/14 grid; bilinear mapping; align_corners false | 32 × 32 for a 448 × 448 canvas |
| Per-branch normalization | Shared per-branch normalization; per-branch unit normalization; joint coefficients are square roots of distance weights | No learned scale or fusion parameter |
| Normal reference memory and scoring | Shared memory and scoring; all aligned support patches; exact nearest-neighbor matching | Fixed bank per category, seed and K |
| Anomaly map and image score | Shared output mapping; resize to retained H × W canvas; Gaussian standard deviation 4 pixels | Continuous map; maximum image score |
| Target-domain optimization | No target optimization; no target loss, optimizer, learning rate or training epochs | 0 target-trainable parameters |

AnomalyCLIP visual requests layers 6, 12, 18 and 24, uses a DPAM layer setting of 20 and retains the final patch tensor. No text scores enter the model. The fixed KolektorSDD2 canvas is 630 high × 224 wide; other inputs follow the DINOv2-B/14 canvas. WideResNet50-2 is prespecified; DINO ViT-S/8, ConvNeXt-Tiny and Swin-Tiny are exploratory.
...
All DINOv2-S/14 studies use K = 1, 2, 4 and 8. KolektorSDD2 uses the same four budgets for WideResNet50-2. Supports are nested within seed and shared across paired methods. Extra-encoder comparisons on MPDD/BTAD use the four WideResNet50-2 conditions; their wider twelve-condition runs are exploratory. VisA retains the conservative in-domain role; the loaded prompt learner does not enter the visual-only AnomalyCLIP visual path (Section 4.1.1).
...
Table 4. Primary controlled matrix with the DINOv2-S/14 representation.

| Configuration / matching rule | MPDD pixel AP | MPDD SD | BTAD pixel AP | BTAD SD |
| --- | --- | --- | --- | --- |
| Dual-encoder baseline / Joint matching | 0.3729 | 0.0424 | 0.6408 | 0.0134 |
| Dual-encoder baseline / Independent matching | 0.3791 | 0.0434 | 0.6498 | 0.0129 |
| Duplicate-branch control / Joint matching | 0.3649 | 0.0438 | 0.6316 | 0.0136 |
| Duplicate-branch control / Independent matching | 0.3681 | 0.0445 | 0.6379 | 0.0133 |
| Equal-weight replacement / Joint matching | 0.3626 | 0.0433 | 0.6471 | 0.0108 |
| Equal-weight replacement / Independent matching | 0.3736 | 0.0463 | 0.6529 | 0.0086 |
| Balanced replacement / Joint matching | 0.3678 | 0.0409 | 0.6505 | 0.0113 |
| Balanced replacement / Independent matching | 0.3799 | 0.0437 | 0.6583 | 0.0090 |

Pixel AP on the stride-eight canvas grid. SD is the sample standard deviation across the 12 MPDD or 8 BTAD fixed support conditions; it is not a confidence interval or variation across independent test datasets. BTAD uses corrected coordinates. The table contains the eight configurations required by the interaction; single-branch diagnostics are retained in the archive. Bold identifies the Dual-encoder baseline, not a claim of best performance.
...
Table 5. Matching effect (AP under Independent matching minus AP under Joint matching) for the four constructions under identical support conditions.

| Dataset / construction | Point | Bootstrap mean | 95% interval |
| --- | --- | --- | --- |
| MPDD / Dual-encoder baseline | +0.618 | +0.637 | [+0.337, +0.977] |
| MPDD / Duplicate-branch control | +0.321 | +0.349 | [+0.137, +0.580] |
| MPDD / Equal-weight replacement | +1.093 | +1.111 | [+0.701, +1.544] |
| MPDD / Balanced replacement | +1.213 | +1.253 | [+0.839, +1.722] |
| BTAD / Dual-encoder baseline | +0.900 | +0.916 | [+0.732, +1.172] |
| BTAD / Duplicate-branch control | +0.631 | +0.638 | [+0.518, +0.785] |
| BTAD / Equal-weight replacement | +0.582 | +0.617 | [+0.373, +0.889] |
| BTAD / Balanced replacement | +0.780 | +0.826 | [+0.541, +1.171] |

Pixel AP percentage points, stride eight. The matching difference is formed within one construction, so it changes only the selection rule: a positive value means independent matching scores higher than joint matching. Values are condition-averaged observed differences and are shown next to, never replaced by, the bootstrap means. MPDD uses 12 support conditions (seeds 0, 1, 2) and corrected BTAD uses 8 (seeds 0, 1). Bold identifies the Dual-encoder baseline, not a claim of best performance. These are descriptive summaries of the matching rule and are not part of the three adjusted families defined in Section 4.1.3, so no family-adjusted interval is claimed for them.
...
Table 6. Absolute effects of replacing or adding DINOv2-S/14 under each matching rule.

| Dataset | Effect | Point | 95% interval |
| --- | --- | --- | --- |
| MPDD | $E_{Equal,J}$ | −0.224 | [−0.968, +0.478] |
| MPDD | $E_{Equal,L}$ | +0.548 | [−0.092, +1.160] |
| MPDD | $E_{Balanced,J}$ | −0.510 | [−1.135, +0.123] |
| MPDD | $E_{Balanced,L}$ | +0.085 | [−0.434, +0.708] |
| BTAD | $E_{Equal,J}$ | +1.553 | [+0.637, +2.465] |
| BTAD | $E_{Equal,L}$ | +1.503 | [+0.585, +2.495] |
| BTAD | $E_{Balanced,J}$ | +0.974 | [+0.177, +1.803] |
| BTAD | $E_{Balanced,L}$ | +0.854 | [+0.057, +1.753] |
...
Table 7. Direct representation-by-matching interactions for DINOv2-S/14.

| Dataset / contrast | Point | 95% interval | 98.75% interval | Full-pixel point |
| --- | --- | --- | --- | --- |
| MPDD / $I_{Equal}$ | +0.772 | [+0.426, +1.118] | [+0.346, +1.211] | +0.787 |
| MPDD / $I_{Balanced}$ | +0.595 | [+0.322, +0.927] | [+0.189, +1.030] | +0.601 |
| BTAD / $I_{Equal}$ | −0.050 | [−0.192, +0.186] | [−0.235, +0.244] | −0.044 |
| BTAD / $I_{Balanced}$ | −0.119 | [−0.265, +0.110] | [−0.306, +0.167] | −0.117 |
...
Table 8. Full-pixel localization for the prespecified WideResNet50-2 extension.

| Configuration / matching rule | MPDD pixel AP | MPDD pixel AUROC | BTAD pixel AP | BTAD pixel AUROC |
| --- | --- | --- | --- | --- |
| WideResNet50-2 only | 0.3452 | 0.9708 | 0.4449 | 0.9542 |
| Dual-encoder baseline / Joint matching | 0.3622 | 0.9658 | 0.6361 | 0.9745 |
| Dual-encoder baseline / Independent matching | 0.3687 | 0.9688 | 0.6455 | 0.9755 |
| Duplicate-branch control / Joint matching | 0.3510 | 0.9626 | 0.6270 | 0.9734 |
| Duplicate-branch control / Independent matching | 0.3554 | 0.9647 | 0.6335 | 0.9742 |
| Equal-weight replacement / WideResNet50-2 / Joint matching | 0.3960 | 0.9731 | 0.6464 | 0.9743 |
| Equal-weight replacement / WideResNet50-2 / Independent matching | 0.4107 | 0.9768 | 0.6591 | 0.9758 |
| Balanced replacement / WideResNet50-2 / Joint matching | 0.3942 | 0.9736 | 0.6401 | 0.9749 |
| Balanced replacement / WideResNet50-2 / Independent matching | 0.4076 | 0.9773 | 0.6545 | 0.9766 |

All output-canvas pixels, seeds 0 and 1, K = 1 and 4. Each number averages category metrics and the four support conditions. No full-pixel confidence intervals were computed. The WideResNet50-2-only row does not use the AnomalyCLIP visual branch. Bold identifies Dual-encoder baseline, not a claim of best performance.
...
Table 9. Direct matching interactions with WideResNet50-2 replacing DINOv2-S/14.

| Dataset / contrast | Point | 95% interval | 98.75% interval | Full-pixel point |
| --- | --- | --- | --- | --- |
| MPDD / Equal-weight replacement | +0.974 | [+0.602, +1.472] | [+0.514, +1.689] | +1.033 |
| MPDD / Balanced replacement | +0.628 | [+0.279, +1.218] | [+0.162, +1.394] | +0.694 |
| BTAD / Equal-weight replacement | +0.622 | [+0.373, +0.917] | [+0.298, +1.003] | +0.624 |
| BTAD / Balanced replacement | +0.501 | [+0.239, +0.883] | [+0.188, +0.995] | +0.496 |

AP percentage points. All four support conditions use seeds 0 and 1 and K = 1 and 4. Intervals are stride-eight paired percentile intervals, with the four WideResNet50-2 contrasts forming their own adjustment family. The final column has no interval.
...
Table 10. Matched difference between WideResNet50-2 and DINOv2-S/14 interactions.

| Dataset / contrast | Observed difference | Bootstrap mean | 98.75% interval | Full-pixel difference |
| --- | --- | --- | --- | --- |
| MPDD / Equal-weight replacement | +0.207 | +0.326 | [−0.296, +1.118] | +0.264 |
| MPDD / Balanced replacement | +0.081 | +0.139 | [−0.372, +0.806] | +0.075 |
| BTAD / Equal-weight replacement | +0.623 | +0.603 | [+0.255, +0.945] | +0.616 |
| BTAD / Balanced replacement | +0.585 | +0.576 | [+0.204, +1.060] | +0.575 |

AP percentage points; WideResNet50-2 interaction minus DINOv2-S/14 interaction. DINOv2-S/14 is restricted to the identical seeds 0 and 1 and K = 1 and 4. Intervals come from within-replicate differences, not differences of interval endpoints. Four encoder comparisons form a separate exploratory adjustment family. Full-pixel differences have no intervals.
...
Figure 5 continued. (c) Category-level Equal-weight replacement interactions as bootstrap means with individual 95% intervals. MPDD uses the study revision and BTAD the corrected revision. This exploratory comparison has no multiplicity adjustment across categories.
...
Figure 6. Three selected MPDD improvements for the Dual-encoder baseline under independent matching relative to the same baseline under joint matching at seed 0 and K = 4. Values are stored stride-eight per-image AP, rather than the category-pooled AP used in the main tables. Joint and independent heatmaps share one full-map min-max range per row. The cyan independent-matching contour uses 256-bin Otsu thresholding on that normalized score. Red hollow rectangles define identical GT-centered visual crops, enlarged below. GT is used for evaluation and crop placement, never to generate the predicted contour. These are selected extremes, not a random test sample.
...
Figure 7. Two selected MPDD degradations under the Dual-encoder baseline comparison and display protocol of Figure 6. Lower raw scores under independent matching do not guarantee improved pixel ordering or a complete contour. These cases illustrate matching behaviour at the baseline, rather than the representation interaction. Only these two cases and the three Figure 6 cases include the thresholded independent-matching contour; the continuation pages and category appendix compare continuous maps.

![Figure 7 part 2](docs/paper_complete_review_20260920/figures/multimethod/fig7_multimethod_mpdd_s0_k4_metal_plate.png)

Figure 7 continued. MPDD metal plates with scratches. Six configurations share the original-image common valid region of Table 11 at seed 0 and K = 4. Query and GT are followed immediately by the two Dual-encoder baseline columns, then AnomalyDINO without/with reference rotation and the local 128-pixel/official 224-pixel PatchCore configurations. P-AP denotes per-image pixel AP, not category-pooled AP. Each row uses a shared score range across all six maps. Categories were selected to illustrate distinct surface, texture and circuit examples; within each category the three archived samples have the largest AP spread across configurations, with ties broken by sample identity. These selected extremes show contrasting responses, not representative performance or a method ranking. No method-specific thresholded contours are supplied on this page.

![Figure 7 part 3](docs/paper_complete_review_20260920/figures/multimethod/fig7_multimethod_mvtec_s0_k4_grid.png)

Figure 7 continued. MVTec AD grid textures with glue defects. Six configurations share the original-image common valid region of Table 11 at seed 0 and K = 4. Query and GT are followed immediately by the two Dual-encoder baseline columns, then AnomalyDINO without/with reference rotation and the local 128-pixel/official 224-pixel PatchCore configurations. P-AP denotes per-image pixel AP, not category-pooled AP. Each row uses a shared score range across all six maps. Categories were selected to illustrate distinct surface, texture and circuit examples; within each category the three archived samples have the largest AP spread across configurations, with ties broken by sample identity. These selected extremes show contrasting responses, not representative performance or a method ranking. No method-specific thresholded contours are supplied on this page.

![Figure 7 part 4](docs/paper_complete_review_20260920/figures/multimethod/fig7_multimethod_visa_s0_k4_pcb1.png)

Figure 7 continued. VisA printed circuit boards. Six configurations share the original-image common valid region of Table 11 at seed 0 and K = 4. Query and GT are followed immediately by the two Dual-encoder baseline columns, then AnomalyDINO without/with reference rotation and the local 128-pixel/official 224-pixel PatchCore configurations. P-AP denotes per-image pixel AP, not category-pooled AP. Each row uses a shared score range across all six maps. Categories were selected to illustrate distinct surface, texture and circuit examples; within each category the three archived samples have the largest AP spread across configurations, with ties broken by sample identity. These selected extremes show contrasting responses, not representative performance or a method ranking. No method-specific thresholded contours are supplied on this page.

![Figure 7 part 5](docs/paper_complete_review_20260920/figures/multimethod/fig7_multimethod_btad_s0_k4_01.png)

Figure 7 continued. BTAD category 01. Six configurations share the original-image common valid region of Table 11 at seed 0 and K = 4. Query and GT are followed immediately by the two Dual-encoder baseline columns, then AnomalyDINO without/with reference rotation and the local 128-pixel/official 224-pixel PatchCore configurations. P-AP denotes per-image pixel AP, not category-pooled AP. Each row uses a shared score range across all six maps. This page extends the four-dataset comparison of Table 11 to BTAD, whose aggregate interaction direction remains unresolved; within the category the three archived samples have the largest AP spread across configurations, with ties broken by sample identity. These selected extremes show contrasting responses, not representative performance or a method ranking. No method-specific thresholded contours are supplied on this page.
...
| --- | --- | --- | --- | --- |
| Dual-encoder baseline / Independent matching | 0.3698 | 0.6474 | 0.5570 | 0.3763 |
| Dual-encoder baseline / Joint matching | 0.3611 | 0.6380 | 0.5530 | 0.3713 |
...
Category-macro pixel AP, averaged over seeds 0 and 1 and K = 1 and 4. All 864 method-category-condition rows are included. Average retained coverage is 76.56%, 70.49%, 76.56% and 59.07%, respectively. Methods differ in backbone, resolution and augmentation. Each configuration is reported under its own native protocol, keeping its own input resolution, evaluation canvas, rotation setting and reference-bank construction, so the table provides context and is not a ranking. Bold identifies Dual-encoder baseline, not a statistical superiority claim. Per-method protocol (input geometry, canvas, rotation and reference-bank construction) is detailed in Table S2.
...
| --- | --- | --- | --- | --- | --- |
| Dual-encoder baseline / Independent matching | controlled, frozen | 0.3698 | 0.6474 | 0.5570 | 0.3763 |
| Dual-encoder baseline / Joint matching | controlled, frozen | 0.3611 | 0.6380 | 0.5530 | 0.3713 |
...
Mean seconds per complete dataset condition over seeds 0 and 1 and K = 1 and 4; MPDD has 458 queries and BTAD 741. Processing is bank plus retrieval for AnomalyDINO and the combined native stage for PatchCore. GPU cells report the range of process peaks across conditions; each dataset name is printed once for its group. Cached feature extraction and other unrecorded stages are not added; these are not end-to-end per-image latencies. Dual-encoder baseline has no comparable complete historical timing.
...
| --- | --- | --- | --- | --- |
| DINOv2-S/14 | $I_{Equal}$ | +0.539 | [+0.299, +0.823] | Yes |
| DINOv2-S/14 | $I_{Balanced}$ | +0.343 | [+0.129, +0.579] | Yes |
| WideResNet50-2 | $I_{Equal}$ | +0.499 | [+0.212, +0.851] | Yes |
| WideResNet50-2 | $I_{Balanced}$ | +0.367 | [+0.128, +0.653] | Yes |
...
| --- | --- | --- | --- | --- | --- |
| MPDD (development) | $I_{Equal}$ | +0.762 | [+0.426, +1.118] | [+0.346, +1.211] | [+0.324, +1.250] |
| MPDD (development) | $I_{Balanced}$ | +0.615 | [+0.322, +0.927] | [+0.189, +1.030] | [+0.152, +1.081] |
| BTAD (external frozen validation) | $I_{Equal}$ | −0.020 | [−0.192, +0.192] | [−0.237, +0.238] | [−0.245, +0.261] |
| BTAD (external frozen validation) | $I_{Balanced}$ | −0.087 | [−0.258, +0.109] | [−0.300, +0.179] | [−0.313, +0.201] |
| MVTec AD (external frozen validation) | $I_{Equal}$ | +0.432 | [+0.342, +0.530] | [+0.324, +0.553] | [+0.309, +0.570] |
| MVTec AD (external frozen validation) | $I_{Balanced}$ | +0.402 | [+0.313, +0.503] | [+0.297, +0.529] | [+0.287, +0.534] |
| VisA (in-domain frozen validation) | $I_{Equal}$ | +0.927 | [+0.788, +1.064] | [+0.754, +1.112] | [+0.738, +1.114] |
| VisA (in-domain frozen validation) | $I_{Balanced}$ | +0.810 | [+0.673, +0.959] | [+0.631, +1.011] | [+0.611, +1.018] |

AP percentage points, stride eight, 1000 paired bootstrap replicates. The estimate is the mean of the replicate distribution, not the condition-averaged observed difference used in Table 7; on MPDD the two differ by 0.01 point. The 98.75% level is a Bonferroni adjustment over the four MPDD and BTAD cells and the 99.375% level over the eight cells this table covers. The BTAD row pools the study's canonical-mask series rather than the corrected geometry used in Section 4.2.1, which moves the point estimate by about 0.03 point without changing the judgement. Stride-one point estimates are now available for all four datasets (+0.787 and +0.601 points on MPDD, −0.045 and −0.115 on BTAD, +0.423 and +0.386 on MVTec AD, +0.896 and +0.760 on VisA, each pair ordered as $I_{Equal}$ then $I_{Balanced}$). KolektorSDD2 is reported separately in Table 14 and is not part of this family. Each dataset name is printed once for its two rows.
...
Table 16. Interactions of the five frozen encoders, in the four-condition scope shared with DINOv2-S/14 and WideResNet50-2 and in the wider twelve-condition scope.

| Encoder (conditions) | MPDD $I_{Equal}$ | MPDD $I_{Balanced}$ | BTAD $I_{Equal}$ | BTAD $I_{Balanced}$ |
| --- | --- | --- | --- | --- |
| DINOv2-S/14 (4) | +0.695 [+0.193, +1.160] | +0.547 [+0.078, +0.962] | +0.029 [−0.189, +0.293] | −0.052 [−0.273, +0.208] |
| WideResNet50-2 (4) | +1.020 [+0.514, +1.689] | +0.686 [+0.162, +1.394] | +0.631 [+0.298, +1.003] | +0.524 [+0.188, +0.995] |
| DINO ViT-S/8 (4) | +0.811 [+0.246, +1.362] | +0.488 [−0.136, +1.091] | +0.583 [+0.172, +1.056] | +0.224 [−0.158, +0.649] |
| DINO ViT-S/8 (12) | +0.995 [+0.529, +1.504] | +0.747 [+0.290, +1.309] | +0.441 [+0.067, +0.829] | +0.174 [−0.138, +0.516] |
| ConvNeXt-Tiny (4) | −0.018 [−0.358, +0.298] | +0.076 [−0.315, +0.588] | +0.278 [+0.152, +0.494] | +0.422 [+0.158, +0.872] |
| ConvNeXt-Tiny (12) | −0.038 [−0.266, +0.170] | +0.036 [−0.235, +0.443] | +0.178 [+0.090, +0.315] | +0.226 [+0.063, +0.491] |
| Swin-Tiny (4) | +1.253 [+0.644, +2.056] | +0.745 [+0.055, +1.425] | +0.488 [+0.217, +0.906] | +0.465 [+0.171, +0.955] |
| Swin-Tiny (12) | +1.397 [+0.803, +2.215] | +0.939 [+0.355, +1.620] | +0.298 [+0.073, +0.578] | +0.180 [−0.029, +0.442] |

AP percentage points, stride eight; each cell gives the replicate mean followed by the 98.75% paired interval, the Bonferroni level for that encoder's four cells in that scope. Cells marked (4) pool seeds 0 and 1 with K = 1 and 4, the primary scope shared with DINOv2-S/14 and WideResNet50-2, so their paired differences against DINOv2-S/14 are same-condition pairs; cells marked (12) pool seeds 0, 1, 2 with K = 1, 2, 4 and 8, the wider scope recorded for the three additional encoders. Both scopes are given because they change some of the zero-exclusion decisions. No family adjustment covers the additional encoders as a group, because DINO ViT-S/8, ConvNeXt-Tiny and Swin-Tiny were added after the DINOv2-S/14 and WideResNet50-2 results were known.
...
| --- | --- | --- | --- | --- | --- |
| MPDD $I_{Equal}$ | +0.679 | 0.221 | Yes | 7 of 8 | 0.48 |
| MPDD $I_{Balanced}$ | +0.548 | 0.159 | Yes | 6 of 8 | 0.37 |
| BTAD $I_{Equal}$ | +0.125 | 0.136 | No | 3 of 8 | 0.68 |
| BTAD $I_{Balanced}$ | +0.036 | 0.129 | No | 0 of 8 | 0.65 |
...
| Correspondence | $I_{Equal}$ point | $I_{Equal}$ 98.75% | $I_{Balanced}$ point | $I_{Balanced}$ 98.75% |
| --- | --- | --- | --- | --- |
...
| --- | --- | --- | --- |
| Dual-encoder baseline / Joint matching | 197.118 | 196.823–197.424 | 2376.8 |
| Dual-encoder baseline / Independent matching | 195.300 | 195.162–195.491 | 2376.8 |
...
Time: median and observed min–max across three repeat-level sums of six units. Memory: median of 18 timed unit peaks. Seed 0; K = 1, 4; three MPDD categories; one warm-up per unit. Timing boundaries and native-protocol differences are specified in Section 4.2.4 and Figure S5. Bold marks the Dual-encoder baseline configurations, not the fastest or best result.
...
Table 21. Image-level and pixel-level point estimates for the Dual-encoder baseline configurations under matched support conditions.

| Dataset | Configuration | Image AUROC | Image AP | Pixel AUROC | Pixel AP |
| --- | --- | --- | --- | --- | --- |
| MPDD | Dual-encoder baseline / Joint matching | 0.7809 | 0.7999 | 0.9665 | 0.3596 |
| MPDD | Dual-encoder baseline / Independent matching | 0.7848 | 0.8093 | 0.9696 | 0.3649 |
| BTAD | Dual-encoder baseline / Joint matching | 0.9324 | 0.9414 | 0.9725 | 0.6339 |
| BTAD | Dual-encoder baseline / Independent matching | 0.9321 | 0.9468 | 0.9736 | 0.6434 |
| MVTec AD | Dual-encoder baseline / Joint matching | 0.9572 | 0.9773 | 0.9663 | 0.5548 |
| MVTec AD | Dual-encoder baseline / Independent matching | 0.9582 | 0.9771 | 0.9677 | 0.5569 |
| VisA | Dual-encoder baseline / Joint matching | 0.9029 | 0.9049 | 0.9736 | 0.3634 |
| VisA | Dual-encoder baseline / Independent matching | 0.9041 | 0.9038 | 0.9761 | 0.3628 |

Seeds 0 and 1; K = 1 and 4; equal category means followed by equal condition means. Categories: MPDD 6, BTAD 3, MVTec AD 15, VisA 12. Pixel metrics use the retained output canvas at stride eight; BTAD uses corrected coordinates. These are current controlled-pipeline point estimates, not the common-original-region metrics of Tables 11 and 12. No image-level confidence interval is available. Bold identifies the Dual-encoder baseline configurations, not statistically optimal results.
...
[23] G. Xie, X. He, D. Fan, S. Li, and Y. Liu, 'Hyper-FSAD: Training-Free and Language-Free Few-Shot Anomaly Detection via Sparse Hyper Matching,' arXiv:2605.10628v2, 2026. https://arxiv.org/abs/2605.10628v2

[24] L. Yi, Z. Chen, G. Wu, J. Ding, X. Wang, and Z. Ning, 'ReMem: A Dynamic Memory Evolution Detector for Zero-Shot Anomaly Detection,' Proc. IEEE/CVF Conf. Comput. Vis. Pattern Recognit. Findings, pp. 7697–7705, 2026. https://openaccess.thecvf.com/content/CVPR2026F/html/Yi_ReMem_A_Dynamic_Memory_Evolution_Detector_for_Zero-Shot_Anomaly_Detection_CVPRF_2026_paper.html

[25] J.-Z. Tang, P.-H. Huang, M.-C. Chang, C.-F. Hsu, and J.-L. Li, 'DuoAD: Leveraging [CLS] Dual Characteristics for Training-Free Few-Shot Anomaly Detection,' arXiv:2607.23924v1, 2026. https://arxiv.org/abs/2607.23924v1

[26] A. Radford et al., 'Learning transferable visual models from natural language supervision,' in Proc. 38th Int. Conf. Mach. Learn., vol. 139, 2021, pp. 8748-8763. https://proceedings.mlr.press/v139/radford21a.html

[27] J. Jeong, Y. Zou, T. Kim, D. Zhang, A. Ravichandran, and O. Dabeer, 'WinCLIP: Zero-/few-shot anomaly classification and segmentation,' in Proc. IEEE/CVF Conf. Comput. Vis. Pattern Recognit. (CVPR), 2023, pp. 19606-19616. https://doi.org/10.1109/cvpr52729.2023.01878

[28] X. Li, Z. Zhang, X. Tan, C. Chen, Y. Qu, Y. Xie, and L. Ma, 'PromptAD: Learning prompts with only normal samples for few-shot anomaly detection,' in Proc. IEEE/CVF Conf. Comput. Vis. Pattern Recognit. (CVPR), 2024, pp. 16838-16848. https://openaccess.thecvf.com/content/CVPR2024/html/Li_PromptAD_Learning_Prompts_with_only_Normal_Samples_for_Few-Shot_Anomaly_CVPR_2024_paper.html

[29] J. Zhu and G. Pang, 'Toward generalist anomaly detection via in-context residual learning with few-shot sample prompts,' in Proc. IEEE/CVF Conf. Comput. Vis. Pattern Recognit. (CVPR), 2024, pp. 17826-17836. https://doi.org/10.1109/cvpr52733.2024.01688

[30] W. Ma et al., 'AA-CLIP: Enhancing zero-shot anomaly detection via anomaly-aware CLIP,' in Proc. IEEE/CVF Conf. Comput. Vis. Pattern Recognit. (CVPR), 2025, pp. 4744-4754. https://doi.org/10.1109/cvpr52734.2025.00447

[31] J. Zhu, Y.-S. Ong, C. Shen, and G. Pang, 'Fine-grained abnormality prompt learning for zero-shot anomaly detection,' in Proc. IEEE/CVF Int. Conf. Comput. Vis. (ICCV), 2025, pp. 22241-22251. https://doi.org/10.1109/iccv51701.2025.02065

[32] Z. Gu, B. Zhu, G. Zhu, Y. Chen, M. Tang, and J. Wang, 'UniVAD: A training-free unified model for few-shot visual anomaly detection,' in Proc. IEEE/CVF Conf. Comput. Vis. Pattern Recognit. (CVPR), 2025, pp. 15194-15203. https://openaccess.thecvf.com/content/CVPR2025/html/Gu_UniVAD_A_Training-free_Unified_Model_for_Few-shot_Visual_Anomaly_Detection_CVPR_2025_paper.html

[33] Y. Wang, J. Peng, J. Zhang, R. Yi, Y. Wang, and C. Wang, 'Multimodal Industrial Anomaly Detection via Hybrid Fusion,' in Proc. IEEE/CVF Conf. Comput. Vis. Pattern Recognit. (CVPR), 2023, pp. 8032–8041. https://openaccess.thecvf.com/content/CVPR2023/html/Wang_Multimodal_Industrial_Anomaly_Detection_via_Hybrid_Fusion_CVPR_2023_paper.html

[34] Y. Lin et al., 'Commonality in Few: Few-Shot Multimodal Anomaly Detection via Hypergraph-Enhanced Memory,' Proc. AAAI Conf. Artif. Intell., vol. 40, no. 9, pp. 7015–7023, 2026. https://ojs.aaai.org/index.php/AAAI/article/view/37636

[35] X. Chen, X. Wang, Y. Wang, C. Han, and L. Duan, 'Learning Enhanced Representations via Contrasting for Multi-view Outlier Detection,' in Database Systems for Advanced Applications (DASFAA), 2023, pp. 110–120. https://link.springer.com/chapter/10.1007/978-3-031-30678-5_9

[36] J. Božič, D. Tabernik, and D. Skočaj, 'Mixed supervision for surface-defect detection: from weakly to fully supervised learning,' Comput. Ind., vol. 129, Art. no. 103459, 2021. https://doi.org/10.1016/j.compind.2021.103459

[37] J. Johnson, M. Douze, and H. Jegou, 'Billion-scale similarity search with GPUs,' IEEE Trans. Big Data, vol. 7, no. 3, pp. 535-547, 2021. https://doi.org/10.1109/tbdata.2019.2921572
...
Table S1. Restricted harmonised-geometry comparison at seed 0 and K = 1.

| Dataset | Configuration | Pixel AP | Pixel AUROC | 95% AP interval |
| --- | --- | --- | --- | --- |
| MPDD | Dual-encoder baseline / Joint matching | 0.3123 | 0.9497 | [0.2935, 0.3366] |
| MPDD | Dual-encoder baseline / Independent matching | 0.3167 | 0.9527 | [0.2980, 0.3409] |
| MPDD | AnomalyDINO | 0.2846 | 0.9460 | [0.2676, 0.3115] |
| MPDD | AnomalyDINO + rotation | 0.2929 | 0.9471 | [0.2763, 0.3195] |
| MPDD | PatchCore 448 | 0.2110 | 0.9466 | [0.1978, 0.2272] |
| BTAD | Dual-encoder baseline / Joint matching | 0.6174 | 0.9722 | [0.5594, 0.6642] |
| BTAD | Dual-encoder baseline / Independent matching | 0.6278 | 0.9738 | [0.5701, 0.6735] |
| BTAD | AnomalyDINO | 0.5576 | 0.9617 | [0.4992, 0.6058] |
| BTAD | AnomalyDINO + rotation | 0.5881 | 0.9655 | [0.5354, 0.6315] |
| BTAD | PatchCore 448 | 0.3725 | 0.9587 | [0.3315, 0.4290] |
| MVTec AD | Dual-encoder baseline / Joint matching | 0.5582 | 0.9553 | [0.5413, 0.5783] |
| MVTec AD | Dual-encoder baseline / Independent matching | 0.5612 | 0.9570 | [0.5446, 0.5817] |
| MVTec AD | AnomalyDINO | 0.5611 | 0.9506 | [0.5400, 0.5827] |
| MVTec AD | AnomalyDINO + rotation | 0.5582 | 0.9497 | [0.5394, 0.5811] |
| MVTec AD | PatchCore 448 | 0.5050 | 0.9228 | [0.4860, 0.5246] |
| VisA | Dual-encoder baseline / Joint matching | 0.3439 | 0.9544 | [0.3166, 0.3671] |
| VisA | Dual-encoder baseline / Independent matching | 0.3493 | 0.9579 | [0.3215, 0.3721] |
| VisA | AnomalyDINO | 0.3032 | 0.9530 | [0.2754, 0.3278] |
| VisA | AnomalyDINO + rotation | 0.3256 | 0.9549 | [0.2978, 0.3502] |
| VisA | PatchCore 448 | 0.3228 | 0.9525 | [0.2960, 0.3468] |

Short side 448; frozen common valid region and the same rank-based pooling. This covers 36 of the 144 category-condition units. PatchCore 448 was recomputed; the other four configurations reuse compatible predictions. Additional crops may be required to respect the frozen intersection. SubspaceAD, WinCLIP+ and zero-shot AnomalyCLIP are excluded because their square-stretch input paths are outside this subset. Points use full-grid common-region pixels; intervals use stride-eight pixels and 1000 image-bootstrap replicates. Intervals apply only to pixel AP, are individual marginal 95% intervals, and are neither family-adjusted nor paired cross-method difference intervals. Pixel AUROC has point values only. This is not a ranking or a significance comparison. Bold identifies the Dual-encoder baseline configurations, not optimal results; Tables 11 and 12 remain unchanged.
...
Table S2. Input and reference protocols of the configurations in Tables 11 and 12.

| Configuration | Input geometry | Augmentation | Reference construction / scoring |
| --- | --- | --- | --- |
| Dual-encoder baseline / Joint matching | DINOv2-B/14: short side 448; AnomalyCLIP visual: square 518, mapped to DINOv2-B/14 | None | All normal support descriptors; shared neighbor across branches |
| Dual-encoder baseline / Independent matching | Same as Dual-encoder baseline / Joint matching | None | Same bank; independent branch neighbors |
