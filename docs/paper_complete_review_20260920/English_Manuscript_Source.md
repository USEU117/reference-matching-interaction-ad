...
Comparing the equal-weight replacement directly with the dual-encoder baseline does not isolate the contribution of DINOv2-S/14. This comparison changes both the effective weights and the descriptor set. Comparing the equal-weight replacement with the duplicate-branch control instead holds the branch slots and their distance weights fixed. The balanced replacement provides a complementary comparison with the baseline: it preserves the AnomalyCLIP visual weight and the combined weight of the remaining branches. These controls do not establish that equal two-branch weights are universally preferable. They show that the weight change affects the present results and must therefore be separated from the value of the new representation.
...
Full-pixel evaluation checks whether sparse pixel sampling changes the estimated interactions, particularly for small defects. The completed stride-one analysis covers the DINOv2-S/14 constructions under 12 MPDD conditions and 8 BTAD conditions, using canonical-mask geometry for BTAD. The two primary MPDD interactions remain positive with 98.75% intervals excluding zero; their intervals are narrower than at stride eight. The MPDD balanced-replacement effect under joint matching excludes zero at strides one and four but spans zero at stride eight, so not every secondary inference is insensitive to resolution. For BTAD, both canonical-mask stride-one interaction intervals span zero. The corrected-geometry stride-eight intervals also span zero, but these are not a like-for-like grid comparison; we therefore do not infer a BTAD interval-width change. The full-pixel results are supplied with the reproduction materials and do not replace the corrected-BTAD main tables. Sparse sampling remains relevant to the selected qualitative examples: their per-image AP can change markedly when only a few sampled pixels are positive.
