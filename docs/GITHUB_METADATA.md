...
1. 打开 **`https://github.com/USEU117/reference-matching-interaction-ad`** 并登录有权限的账号。
...
> 路径一律用**新名** `D:\STUDY\My_github\reference-matching-interaction-ad`。当前该路径是指向本地物理目录的 junction，两者是同一份文件，用哪个名字都能跑。

```powershell
# 1) 进入仓库（新名路径）
Set-Location 'D:\STUDY\My_github\reference-matching-interaction-ad'

# 2) 把 origin 指到改名后的远端（旧 URL 会重定向，但显式改成新 URL 更稳）
git remote set-url origin https://github.com/USEU117/reference-matching-interaction-ad.git
