...
Figure 3. Fixed representation constructions and the contrasts built from them. (a) A1, DUP, TRI and BAL with their distance weights; a slot keeps the same colour across panels. (b) A1 to DUP varies only the effective weight of B and is therefore the weight-confounding control; DUP to TRI puts a real encoder into the slot at an unchanged weight; A1 to BAL keeps the non-C family total and the C weight equal. (c) The four contrasts estimated in this paper. The TRI-to-A1 difference moves both factors at once and is never attributed to S alone. The extra slot is instantiated by five frozen encoders: S and D were pre-specified, while E1, E2 and E3 were added after the S and D results were known and are exploratory transfer checks. These are analysis configurations in one frozen pipeline, not trained networks.
