...
**Figure 6 (part 1 / part 2).** Qualitative MPDD improvements of A1 L over A1 J at seed 0, K = 4. Each row shows the query, the ground-truth mask for visual reference, the two continuous anomaly heatmaps on one shared per-case min-max range, an L contour obtained only for visualization by 256-bin Otsu thresholding, and the same GT-defined crop enlarged below.
