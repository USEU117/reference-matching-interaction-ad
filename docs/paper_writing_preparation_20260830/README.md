...
24. [TCRR 区域重排实验：完整结果与下一算法路线](18_TCRR_EXPERIMENT_RESULT_AND_NEXT_ALGORITHM_CN_20260903.md) —— 已实际开发并完成 v8 R0/R0b/R1/R2/R3。MPDD 3 seeds × 3 shots 上 Pixel-AP 稳定提升约 +0.032～+0.036，空间错位对照证明区域互补信号真实；但一次性 BTAD（−0.00695）和 MVTec（−0.00582）均失败，因此 v8 不能升级为论文主方法。正常参考校准、只增益、安全回退的 v9-R0 也已执行：总体 +0.02327，但 metal_plate −0.01823 未过安全线，按规则归档。
25. [多路线算法创新探索与并行执行任务书](19_MULTI_ROUTE_INNOVATION_EXPLORATION_HANDOFF_CN_20260903.md) —— 面向下一位 AI 的持续执行规格，跳出文本融合，覆盖跨参考一致性记忆（CRAM）、多视图等变稳定（MESP）、对齐后位置条件记忆（CAPM）、正常参考区域风险校准（NORC）、频谱纹理残差（STR）和自监督部件关系图（SPRG）。逐条给出科学问题、最小实现、强对照、淘汰门、统一 seed 确认和新外部验证协议，要求一次性广度筛选后再深入 1–2 条路线。
