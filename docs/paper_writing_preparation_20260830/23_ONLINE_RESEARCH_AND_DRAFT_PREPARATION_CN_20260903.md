...
状态：可供外部评审/作者审阅；作者已明确以“中科院目标分区（应用/实证型）”为目标，待最终选刊后可据此生成英文论文 V0.1
...
6. 当前文献候选库已经从 24 篇扩充为 30 篇，其中 22 篇为 2024--2026 年，占 73.3%，达到外部评审提出的“近三年至少七成”的暂定比例要求。
7. 作者将目标期刊限定为 **中科院目标分区（应用/实证型）**。这降低的是选刊门槛，不改变创新事实：当前稿应按“应用型、实证型、可复现”路线组织，不能把基础对齐、归一化或固定拼接改名为新算法模块。
...
| 2024--2026 年研究现状 | 外部评审要求近三年高质量文献占多数 | 已检索 CVF、ICLR、OpenReview、PMLR、出版社页面 | Introduction 第 2--4 段；Related Work |
...
| 中科院分区、是否在目标学科、版面费 | 每年会变化，不能凭旧印象 | 已按“中科院目标分区（应用/实证型）”初筛；投稿前须用学校认可的中科院分区表终验 | 选刊决策，不写进论文 |
...
| 失败边界与部署代价分析 | 较强 |
| 适合论文类型 | 应用型、实证型、可复现性强调的 论文 |

这意味着论文仍然可以写，但题目、摘要和贡献不能假装成高复杂度新网络。若外部评审坚持“必须有明确算法模块创新”，则需要另立新实验任务；不能只靠重新命名把 A1 变成 DCFNet。

---

## 9. 主图需要怎样准备

仓库已有 Fig01 内容草图及 SVG/PDF/PNG，但最终提交版仍建议按AI 辅助评审要求在 PPT、Visio 或 draw.io 中人工重绘并保留可编辑源文件。
...
1. 论文定位是否接受“controlled empirical study”，还是外部评审明确要求继续增加算法创新；
...
本轮没有直接生成完整论文，目的是先让作者和外部评审确认：论文究竟按“受控实证研究”投稿，还是必须再做一个通过验证的新算法模块。这个决定会直接改变题目、摘要、Introduction gap、主图和贡献列表。
