...
| **B** 几何对应审计 + 学习式对应替换 | `LC/B_correspondence/` | `B1_SUMMARY.json`、`B2_SUMMARY.json`、`B2_SUMMARY_btad.json`、`REPORT_CN.md`、`audit_metrics.csv`、`variant_metrics{,_btad}.csv`、`interaction_by_variant.csv`、`ot_info.csv`、`ot_sensitivity{,_btad}.csv`、`VB_2_IDENTITY_GATE.json`、`VB_2_OT_IDENTITY_GATE.json` | `…\b1_correspondence_audit.py`；`…\b2_learned_correspondence.py --mode variants`；BTAD：`--mode btad --variants identity procrustes shuffled ot_sinkhorn --seeds 0 1 --shots 1 4 --replicates 1000 --ot-factor 0.1 --ot-factors 0 0.05 0.5` | 完成但有已知瑕疵（OT 软混合下两条区间跨零；BTAD ε 网格未落逐单元诊断） |
...
| `NEW/RUN_SUMMARY.json` | `created_utc = 2026-09-14`；`stages` 仅到 `S9_resource_measurement`；`artefact_count = 829`、`artefact_bytes = 8015572367`、`selfcheck = 71/71` | 现值：`SELFCHECK.json` 记 `checks=71, passed=69`；09-15 起的新阶段（种子扩展、四数据集泛化、KSDD2 确认）未登记 ⇒ 重生而非手改。详见 `NEW/STALE_20260919.md` |
