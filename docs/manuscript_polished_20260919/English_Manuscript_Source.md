...
3. **Evidence that representation–matching interactions vary by encoder and dataset.** The primary experiments separate the two outcomes above on MPDD and BTAD. Validation on MVTec AD and VisA, a separately specified KolektorSDD2 confirmation, and matched encoder substitutions extend the evidence while revealing its limits. DINOv2-S has an unresolved interaction direction on BTAD, whereas WideResNet50-2 has positive interactions on both primary datasets; ConvNeXt provides an inconclusive case on MPDD. These findings establish a bounded design insight: representation composition and normal-reference selection need to be assessed jointly. Support-set and correspondence analyses further identify conditions that warrant sensitivity checks.
...
Three lines of research inform the attribution of gains in a frozen multi-encoder system: methods for modeling normal appearance and retaining reference features, foundation-model fusion, and neighborhood consistency across representations. We review their capabilities and identify the specific comparison that remains necessary in the present setting.

### 2.1 Normal Appearance Models and Feature Memories

Learned normal models identify anomalies through deviations from expected appearance or feature structure. SLSG combines generative pretraining, simulated anomalies and graph-based embedding modeling [13]. RealNet uses diffusion-based anomaly synthesis with feature and residual selection [14]. PaDiM instead fits multivariate Gaussian distributions to pretrained patch embeddings [15]. These methods offer different ways to estimate normality, with corresponding training, adaptation and sample-size requirements.
...
### 2.3 Neighborhood Consistency across Representations
...
AP percentage points; observed means over seeds 0, 1 and K = 1, 4 in a separate correspondence-sensitivity implementation. IQR is the interquartile range of support costs. Paired image-bootstrap intervals are unavailable for these variants; the means provide descriptive sensitivity evidence and are not interchangeable with primary-pipeline estimates.

All reported MPDD mean interactions are positive, including those under soft transport and exact assignment. Table 11 averages four support conditions and reports descriptive points only. Paired image-bootstrap confidence intervals are unavailable for these variants, so their signs do not establish a population interaction direction.

The positive MPDD points show that the observed sign persists beyond the canvas convention in the tested conditions. A consistent orthogonal transformation preserves within-branch distances, making its unchanged result a numerical consistency check. Soft transport also changes the representation through descriptor averaging, so it probes both representation and correspondence sensitivity.

BTAD covers the same principal variants and transport regularization grid. Its observed means are reported alongside MPDD. Paired image-bootstrap analysis is needed to assess uncertainty in both datasets before drawing statistical robustness conclusions.
...
The results demonstrate why this distinction matters. DINOv2-S shows positive matching interactions on MPDD, MVTec AD and VisA, while its interaction direction remains unresolved on BTAD despite positive absolute representation effects. WideResNet50-2 yields positive interactions on both primary datasets, and the separately specified KolektorSDD2 confirmation meets all four directional criteria. Encoder and support-set analyses identify substantial variation; correspondence transformations provide additional descriptive sensitivity evidence.

The resulting design guidance is to evaluate representation composition and reference selection together under explicit weight controls. Its scope remains conditional on the tested encoders, support sets and dataset provenance. Future work should extend prospective comparisons to additional production settings, complete paired uncertainty analysis for correspondence variants, and evaluate calibrated thresholds and synchronized deployment costs. These steps would connect the controlled attribution of fusion gains to practical inspection decisions.
