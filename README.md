...
| Support-set seed variance (8 seeds) | MPDD `I_TRI` support-set sd (of the eight per-seed replicate means) / test-side median 95% half-width = 0.480, 7/8 exclude zero, all same sign; BTAD signs unstable | `experiments/dynamic_fusion/seeds_extension_20260917/interaction_seed_variance.json` |
...
Citation metadata: [CITATION.cff](CITATION.cff). Third-party datasets, weights and method provenance: [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md).

Manuscript: the current authoritative draft is [docs/paper_complete_review_20260920/Reference_Matching_Complete_English_20260923.docx](docs/paper_complete_review_20260920/Reference_Matching_Complete_English_20260923.docx), built by `scripts/paper_complete_review_20260920/build.py` from `{manuscript.md, results.md, tables.json, figures.json, references.json}`; as of 2026-09-23 it measures **55 pages / 23 tables / 8 main figures plus S1–S6 (27 embedded images) / 152 native math objects / 12 numbered equations / 34 references / 19,434 words**. The matching figure deck is `docs/paper_complete_review_20260920/All_Figures_Complete_20260923.pptx` (63 slides). The previous draft of the same chain (`…20260920.docx`, 47 pages / 20 tables / 22 embedded images / 142 math objects / 17,221 words) is kept on disk as the pre-expansion anchor. The older chain [docs/manuscript_reference_matching_20260914/](docs/manuscript_reference_matching_20260914/) (18 tables / 8 figures, `build_validation.json`) is **superseded** — see its `SUPERSEDED_20260921.md`. **Submission status: in preparation / under review — not published**; citation metadata will be added once the venue or preprint is fixed. **Document availability**: the authoritative manuscript docx is now **shipped with this repository** (tracked in git); the replication package (`dist/replication_package_20260920/`, 2488 files / 470.7 MB ≈ 449 MiB) is distributed via GitHub Release / Zenodo and is **not** tracked in git.
...
| **支持集 seed 方差（8 seeds）** | MPDD `I_TRI` 支持集 sd（八个逐 seed 复现均值的 sd）/ 测试侧中位 **95%** 半宽 = **0.480**，7/8 排除零且全同号；BTAD 不同号 | `experiments/dynamic_fusion/seeds_extension_20260917/interaction_seed_variance.json` |
...
`dist/replication_package_20260920/` — **2488 个文件 / 470.7 MB，被 `.gitignore` 的 `dist/` 排除、未纳入 git**（分发走 GitHub Release / Zenodo）；包内有 `SHA256SUMS`、`SOURCE_COMMIT.txt`、`requirements_lock.txt`。

### 许可与引用

代码：MIT，见根 [LICENSE](LICENSE)。数据集许可各自独立（CC BY-NC-SA 4.0 / CC BY-SA 4.0 / CC BY 4.0，见 [data/README.md](data/README.md)），**不在再分发范围**。

引用元数据见 [CITATION.cff](CITATION.cff)；第三方数据集、权重与方法来源见 [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md)。

论文当前权威稿在 [docs/paper_complete_review_20260920/Reference_Matching_Complete_English_20260923.docx](docs/paper_complete_review_20260920/Reference_Matching_Complete_English_20260923.docx)，由 `scripts/paper_complete_review_20260920/build.py` 从 `{manuscript.md, results.md, tables.json, figures.json, references.json}` 构建；2026-09-23 实测 **55 页 / 23 表 / 8 主图 + S1–S6（27 内嵌图）/ 152 原生数学对象 / 12 编号公式 / 34 文献 / 19,434 词**；配套图件 deck 为 `docs/paper_complete_review_20260920/All_Figures_Complete_20260923.pptx`（63 页）。同链上一版（`…20260920.docx`，47 页 / 20 表 / 22 内嵌图 / 142 数学对象 / 17,221 词）作为扩版前锚点保留在盘。旧链 [docs/manuscript_reference_matching_20260914/](docs/manuscript_reference_matching_20260914/)（18 表 / 8 图，见其 `build_validation.json`）已标 **superseded**，见同目录 `SUPERSEDED_20260921.md`。**投稿状态：准备投稿 / 审阅中，尚未发表**；正式引用信息待定稿后补充。**文档获取**：权威稿 docx **已随本仓库提供**（纳入 git 跟踪）；复现包（`dist/replication_package_20260920/`，2488 个文件 / 470.7 MB ≈ 449 MiB）走 GitHub Release / Zenodo，**未入库**。
